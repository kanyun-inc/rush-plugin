...
`rush-ai` is the CLI you use. You don't need to install it — every command in this skill runs via `npx -y rush-ai@latest`, so `npx` handles download + caching on first run. Its own `skill hand-off` subcommand prints an authoritative playbook — **read it first** if you haven't:

```bash
npx -y rush-ai@latest skill hand-off
```
...
- Questions about Rush itself ("rush-ai 怎么用"、"rush 是什么") — answer from docs / `npx -y rush-ai@latest --help`.
- When the user is clearly iterating on a project already cloned locally — use `npx -y rush-ai@latest task send <id>` against the existing task id instead of creating a new one.
...
If the response says not authenticated, **stop and ask the user** to run `npx -y rush-ai@latest auth login` themselves (it opens a browser). Do not try to log in on their behalf, do not fall through to `task create` — it will 401.
...
Discover more with `npx -y rush-ai@latest agent list --default --json` if needed.
...
```bash
npx -y rush-ai@latest task create -a web-builder -p "<prompt synthesized from context>" --json
```
...
REPO_URL=$(npx -y rush-ai@latest task status <id> --json | jq -r '.gitRepoUrl')
...
- The task id (for future `npx -y rush-ai@latest task send <id>` iterations, if they want more cloud-side changes)
...
- **Heavy regeneration** ("redo the hero section completely", "regenerate the whole pricing page"): `npx -y rush-ai@latest task send <id>` so the Rush agent rewrites it, then `git pull` locally. Only use `task send` against the original task id, not a new one.
...
# 2. create
npx -y rush-ai@latest task create -a web-builder \
  -p "做一个公司官网首页，包含导航、Hero、核心特性、客户评价和底部联系信息，风格大气专业。" \
  --json
# → {"id": "lodig8oknq0r", "status": "pending", ...}

# 3. poll
npx -y rush-ai@latest task status lodig8oknq0r --json
# ...wait...
